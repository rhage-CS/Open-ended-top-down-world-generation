...
for (let y = 0; y < height; y++) {
  for (let x = 0; x < width; x++) {
    ground[y][x] = 0; // plain grass
  }
}

// Dirt path at row 12, x=8 to x=20 with transitions
// Main path
for (let x = 8; x <= 20; x++) {
...
setGround(7, 12, 36); // lower-left corner of dirt patch
setGround(8, 12, 25); // already set

// Right transition
setGround(20, 12, 25); // already set
setGround(21, 12, 38); // lower-right corner
...
// 4. Complete roof run from indices 48-67 at y=5, x=13 to x=16
setObject(13, 5, 48); // left edge
setObject(14, 5, 49); // fill
setObject(15, 5, 51); // variant fill
setObject(16, 5, 50); // right edge

// 5. Stone arch (wide span) at (10,11) and (11,11)
setObject(10, 11, 113); // left half
setObject(11, 11, 114); // right half

// 6. Fence enclosure (closed) at (8,4) to (10,6)
// Top row y=4
...
// 7. Rail fence section at (18,10) to (20,10)
...
// 8. Large assembly structure at (2,2) to (4,4), 3x3
// Top row y=2
...
// 9. Well at (12,6) and (12,7)
setObject(12, 6, 92); // well top
setObject(12, 7, 104); // well bottom

// 10. Signpost at (6,3) and (6,4)
setObject(6, 3, 83); // sign board
setObject(6, 4, 71); // post

// 11. Tree group (block-locked 3x3 green variant) at (19,0) to (21,2)
